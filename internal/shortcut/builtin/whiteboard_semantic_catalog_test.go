...
func TestCrossPlatformCoverageWhiteboardSemanticCatalogExactlyCoversRegisteredSurface(t *testing.T) {
	raw, err := os.ReadFile("../semantic_catalog_whiteboard.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	for command, item := range registered {
		...
		if !record.Public || item.Hidden || !shortcut.InPublicCatalog("whiteboard", command) {
			t.Errorf("%s did not enter the public surface", command)
		}
		if !record.Public {
			hidden++
		} else {
		...
		if item.Availability != shortcut.AvailabilityAvailable || record.Availability != shortcut.AvailabilityAvailable || item.Risk != record.Risk {
			t.Errorf("%s availability/risk=%q/%q", command, item.Availability, item.Risk)
		}
		if item.Contract.Empty() || item.Contract.Result == nil || item.Contract.Pagination != nil || strings.TrimSpace(item.Safety.Effect) == "" || item.OutputRollout != output.RolloutUnifiedActive || item.Contract.Interface == nil || item.Contract.Interface.Availability != "available" || strings.TrimSpace(item.Contract.Interface.Reason) == "" {
